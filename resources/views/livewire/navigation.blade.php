<nav class="bg-gray-800" x-data="{open:false}">
      <div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
            <div class="relative flex items-center justify-between h-16">
                ...
                {{-- Si el usuario está autentificado, se mostrará lo siguiente. --}}
                @auth
                ...
                <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                  ...
                  <!-- Profile dropdown -->
                  <div class="ml-3 relative" x-data="{open: false}"> {{-- Agregamos una variable tipo objeto "open" para tratar como componente al div según alpine. Los divs dentro de este contenedor pueden alterar dicha variable. --}}
                    <div>
                      {{-- Alpine con x-on maneja eventos, en este caso usamos el evento click sobre dicho boton. --}}
                      <button x-on:click="open=true" type="button" class="bg-gray-800 flex text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white" id="user-menu-button" aria-expanded="false" aria-haspopup="true">
                        ...
                        <img class="h-8 w-8 rounded-full" src="{{auth()->user()->profile_photo_url}}" alt=""> {{-- Cambiamos foto de perfil por el del usuario autentificado --}}
                      </button>
                    </div>
                    ...
                    {{-- "x-show" oculta o muestra un div (true muestra, false oculta). En este caso usamos el valor de la variable "open" que puede ser true o false. --}}
                    {{-- click.away es un método para el click que se haga en cualquier otro div menos en el que está especificado. --}}
                    ...
                    <div x-show="open" x-on:click.away="open=false"class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
                      ...
                      <a href="{{route('profile.show')}}" class="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="user-menu-item-0">Tu perfil</a> {{-- Indicamos ruta para ver perfil del actual usuario. --}}
                      
                      @can('admin.home')  {{-- can verifica si tenemos el permiso necesario para ver esta opción.. El permiso 'admin.home está especificado en el seeder RoleSeeder.' --}}
                      ...
                      this.closest('form').submit();">Cerrar sesion</a> {{-- Con el evento event.preventDefault evitamos la petición a través del enlace y se permite enviar el formulario. --}}
                      </form>
                    </div>
                  </div>
